...
frame_t *obtain_free_frame(){
	...
	if( pt[q].pt_dirty == 1){
		...
		write_bs((char *) fid_2_vaddr(pg_frameptr->fid),store,offset); // consider the possibility that pid != currpid; write from page frame's address
		...
		for(gca_plus=0; gca_plus < NFRAMES; gca_plus++){
			...
			if(frametab[gca_curr].ftype == PG_TYPE){
				...
				if( (ptptr[ptnum].pt_acc==0) && (ptptr[ptnum].pt_dirty==0)){
					...
					kprintf("getpageframe_gca error, pt_acc=0, pt_dirty=1");
					restore(mask);
					return (void *) SYSERR;
				}
			}
