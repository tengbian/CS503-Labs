...
void pagefault_handler(){
	...
	if (ptptr[q].pt_pres != 1){
		...
		if (open_bs(store) != SYSERR){
			kprintf("currpid: %d,bs open success: fid: %d, vaddr: %u\n",currpid,page_frameptr->fid, fid_2_vaddr(page_frameptr->fid));
		}
		read_bs((char *)fid_2_vaddr(page_frameptr->fid),store,offset);// to the addr of page_frame /// Problem Here
		kprintf("currpid: %d,bs_read_success\n",currpid);
		if (close_bs(store)!= SYSERR){
			kprintf("bs_close_success\n");
		}
