...
struct	gprocent grouptab[NGROUP]; /* Group table	*/
...
{	
	...
	resume(create((void *)startup, INITSTK, PS1, INITPRIO,
...
local process	startup(void)
{
	...
	resume(create((void *)main, INITSTK,PS1, INITPRIO,
					"Main process", 0, NULL));
...
static	void	sysinit()
{
	...
	struct	gprocent *gptr;		/* Ptr to group table entry */
	...
	prptr->Pi = INT_MAX;
	prptr->PRIOi = INT_MAX - prptr->Pi;
	prptr->blocked = 0;// the first process, treat it as not blocked
